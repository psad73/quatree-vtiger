...
class Vtiger_MailScanner
{
        ...
        foreach ($folders as $lookAtFolder) {
            ...
            foreach ($mailsearch as $messageid) {
                ...
                if ($this->isMessageScanned($mailrecord, $lookAtFolder)) {
                    $this->log("\nMailScanner: Message already scanned [$mailrecord->_subject], IGNORING...\n");
                    unset($mailrecord);
                    continue;
                } else {
                    $this->log("\nMailScanner: New message [$mailrecord->_subject][$mailrecord->_uniqueid], PROCESSING...\n");
                }
    ...
    function applyRule($mailscannerrule, $mailrecord, $mailbox, $messageid)
    {
        ...
        if ($bodyrule) {
            ...
            $this->log("\nMailScanner: Mailbody required for testing, fetching mailbody (msgid: " . $messageid . ").");
        ...
        if ($matchresult) {
            $mailrecord->fetchBody($mailbox->_imap, $messageid);
            $this->log('MailScanner: Taking actions for message [' . $messageid . ']');
