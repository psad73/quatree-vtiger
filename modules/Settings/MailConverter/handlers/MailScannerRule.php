...
class Vtiger_MailScannerRule
{
    ...
    function applyAll($mailrecord, $includingBody = true)
    {
        ...
        if ($this->hasACondition()) {
            $this->log("MailScannerRule: Processing conditions");
            ...
            $this->log("MailScannerRule: No conditions for this email found");
            $matchfound = false;
            if ($this->matchusing == 'OR') {
                $matchfound = true;
    ...
    function apply($subrule, $mailrecord)
    {
        $matchfound = false;
        if ($this->isvalid) {
            $this->log('MailScannerRule: Trying rule: ' . strtoupper($subrule));
    ...
    function find($subrule, $condition, $input, $searchfor)
    {
        ...
        $matchfound = false;
        $matches = false;
        if (preg_match("/^regex[p]?\(.*\)$/i", $searchfor)) {
            $condition = "Regex";
            $searchfor = preg_replace("/\)$/", "", preg_replace("/^regex[p]?\(/", "", $searchfor));
        }
        switch ($condition) {
            case 'Contains':
        ...
        $this->log('MailScannerRule:    condition "' . $condition . '"');
        $this->log('MailScannerRule:    looking for "' . $searchfor . '"');
        $this->log('MailScannerRule:    got input "' . $input . '"');
        $this->log('MailScannerRule:    found "' . $matchfound . '"');
